...
public class CreateDaoTest {
	 ...
	 * @throws Exception
	 * 
	 *             通过jdbc连接数据库，根据参数 tableName(表名) 生成相应的 java源文件。 生成的java文件没有package ，所以 实际使用时，需在生成的文件中自行加上相应的包名。 descFile
	...
	/**
	 * @auther donnie wu 通过jdao.xml配置文件批量生成dao源文件。 如果dao文件已经存在则会在控制台提示并自动跳过该节点进行下一节点的解析。
	 * 
	 */
	@Test
	...
	public static void main(String[] args) throws Exception {
		...
		Class.forName("com.mysql.jdbc.Driver");
		Connection conn = DriverManager.getConnection("jdbc:mysql://10.10.152.178:8660/istar", "abc", "123456");
		String sql = "select * from i_music_xiami_album limit ?,?";
